...
def pin_gen(article_slug, json_article):
    ### gen images
    template = 2
    template = random.randint(1, 2)
    for filename in os.listdir(f'{g.pinterest_tmp_image_folderpath}/tmp'): # clear forlder
        filepath = f'{g.pinterest_tmp_image_folderpath}/tmp/{filename}'
        os.remove(filepath)
    if template == 1: # 4 images
        width = 1024
        height = 1024
        for i in range(4):
    ...
    lines = pin.pin_text_to_lines(text, font, 800)
    for line in lines:
    ...
    for _ in range(10):
        ...
        font = ImageFont.truetype(font_path, font_size)
        _, _, text_w, text_h = font.getbbox(text)
        lines = pin.pin_text_to_lines(text, font, line_w_max)
    ...
    for line in lines:
        ...
        y_cur += font_size
    ###
    export_filepath = pin.pin_image_save(img, export_filename)
...
for cluster_slugs in articles_slugs:
    ...
    ### gen pin image
    pin_image_filepath = pin_gen(article_slug, json_article)
    # quit()
    ### gen pin json
    article_description = ''
    prompt = f'''
