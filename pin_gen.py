...
def pin_image_gen_pumpkin(article_slug, images_tmp_filepaths, export_filename):
...
for i in range(4):
    ...
    for image_tmp_filename in os.listdir(images_tmp_folderpath):
        ...
        images_tmp_filepaths.append(image_tmp_filepath)
### gen pin image
pin_image_filepath = pin_image_gen_pumpkin(article_slug, images_tmp_filepaths, export_filename)
### gen pin json
article_description = ''
prompt = f'''
    Write a description in less that 500 characters for a pinterest pin with the following title: {article_title}.
...
### flowers pins
item_i += 1
for item in flowers_data[:pins_num]:
    pin_gen(item, item_i)
    item_i += 1
    # quit()
