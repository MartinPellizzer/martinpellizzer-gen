...
def images_gen(article_slug, item, dispel=False):
    flower_slug_singular = item['flower_slug_singular']
    flower_slug_plural = item['flower_slug_plural']
    flower_name_singular = item['flower_name_singular']
    flower_name_plural = item['flower_name_plural']
...
def json_gen(article_slug, item):
    keyword_main = f'''{item['flower_name_singular']} aesthetic'''
    keyword_main_slug = f'''{item['flower_slug_singular']}-aesthetic'''
    json_article_filepath = f'''{g.database_folderpath}/json/{article_slug}.json'''
    json_article = io.json_read(json_article_filepath, create=True)
    json_article['article_slug'] = article_slug
    json_article['entity_name'] = item['flower_name_singular']
    json_article['entity_slug'] = item['flower_slug_singular']
...
def html_gen(article_slug, item):
    try: os.mkdir(f'{g.website_folderpath}')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/plants')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/plants/types')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/plants/types/flowers')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/plants/types/flowers/{item["flower_slug_plural"]}')
    except: pass
    keyword_main = f'''{item['flower_name_singular']} aesthetic'''
    keyword_main_slug = f'''{item['flower_slug_singular']}-aesthetic'''
    json_article_filepath = f'''{g.database_folderpath}/json/{article_slug}.json'''
...
def gen(item):
    ...
    images_slug = f'''images/plants/types/flowers/{item['flower_slug_plural']}/aesthetic'''
    try: os.mkdir(f'{g.website_folderpath}/images')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/images/plants')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/images/plants/types')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/images/plants/types/flowers')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/images/plants/types/flowers/{item["flower_slug_plural"]}')
    except: pass
    try: os.mkdir(f'{g.website_folderpath}/images/plants/types/flowers/{item["flower_slug_plural"]}/aesthetic')
    except: pass
    # images_gen(article_slug, item, dispel=False)
    json_gen(article_slug, item)
    html_gen(article_slug, item)
