...
plants_folderpaths = [f.path for f in os.scandir('database/pages/herbs') if f.is_dir()]
...
def gen_image(images, i, preparation_name, image_style, width, height):
    rnd_plant_name_scientific = random.choice(plants_names_scientific).strip()
    ...
    prompt_juggernaut_xi = f'''
        ...
        on a {rnd_table} table, 
        surrounded by dry {rnd_plant_name_scientific} herbs,
        indoor, 
        natural light,
        earth tones,
        neutral colors,
...
def plant_template_1_img_b(data, images, export_file_name):
    ...
    images = []
    image_style = random.choice(['', 'watercolor'])
    plant_name_scientific = data['plant_name_scientific']
    plant_name_common = data['plant_names_common'][0]['name']
    plant_name = random.choice([plant_name_scientific, plant_name_common])
    if random_theme == 0:
        text_color = '#ffffff'
        bg_color = '#000000'    
    else:
    ...
    if random_images_num == 1:
        for i in range(1):
            gen_image_herb(images, i, plant_name_scientific, image_style, 1024, 1024)
        img_0000 = Image.open(images[0])
        img_0000 = img_resize(img_0000, pin_w, pin_w)
        img.paste(img_0000, (0, pin_h//3))
    elif random_images_num == 2:
        for i in range(2):
            gen_image_herb(images, i, plant_name_scientific, image_style, 832, 1216)
        img_0000 = Image.open(images[0])
        img_0001 = Image.open(images[1])
        img_0000 = img_resize(img_0000, pin_w//2, pin_w)
        img_0001 = img_resize(img_0001, pin_w//2, pin_w)
...
def plant_template_1_img_t(data, images, export_file_name):
    random_theme = random.randint(0, 1)
    random_images_num = random.randint(1, 2)
    images = []
    image_style = random.choice(['', 'watercolor'])
...
def plant_template_1_img_c(data, images, export_file_name):
    random_theme = random.randint(0, 1)
    random_images_num = random.randint(1, 2)
    images = []
    image_style = random.choice(['', 'watercolor'])
    ...
    if random_images_num == 1:
        for i in range(2):
            gen_image_herb(images, i, plant_name_scientific, image_style, 1216, 832)
        img_0000 = Image.open(images[0])
        img_0000 = img_resize(img_0000, pin_w, 500)
        img.paste(img_0000, (0, 0))
        img_0001 = Image.open(images[1])
...
def pin_gen_plants_benefits(article_filepath, article_i):
    ...
    img_slug = url.replace('/', '-')
    filename_out = url.replace('/', '-')
    plant_benefits = data['plant_benefits']
    plant_benefits_descriptions = [x['benefit_desc'] for x in plant_benefits if 'benefit_desc' in x]
    if plant_benefits_descriptions:
        random.shuffle(plant_benefits_descriptions)
        plant_benefit_description = plant_benefits_descriptions[0][:490] + '...'
    else:
        plant_benefit_description = ''
    ...
    # gen pins
    if template == '1_img_b':
        img_filepath = plant_template_1_img_b(data, images, filename_out)
    elif template == '1_img_t':
        img_filepath = plant_template_1_img_t(data, images, filename_out)
    else:
        img_filepath = plant_template_1_img_c(data, images, filename_out)
    obj = {
        'slug': plant_slug,
        'name': plant_name_scientific,
        'title': title,
        'url': url,
        'description': plant_benefit_description,
        'img_filepath': img_filepath,
        'board_name': board_name
    }
    json_write(f'{PINTEREST_PINS_IMAGE_FOLDERPATH}/{article_i}.json', obj)
